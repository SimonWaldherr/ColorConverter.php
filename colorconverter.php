...
function parseColor ($input) {
  ...
  if ($geregext !== null) {
    switch ($geregext[1]) {
    case '#':
      return colorconv($geregext[2], 'hex2rgb');
    case 'rgb':
      return [intval(trim($geregext[4]), 10), intval(trim($geregext[5]), 10), intval(trim($geregext[6]), 10)];
    case 'hsl':
      return colorconv([intval(trim($geregext[4]), 10), intval(trim($geregext[5]), 10), intval(trim($geregext[6]), 10)], 'hsl2rgb');
    case 'yuv':
      return colorconv([intval(trim($geregext[4]), 10), intval(trim($geregext[5]), 10), intval(trim($geregext[6]), 10)], 'yuv2rgb');
    default:
      return false;
    }
  }
...
function colorconv ($input, $mode='parse', $html=false) {
  ...
  case 'rgb2hex':
    ...
    break;
  case 'rgb2yuv':
    $output = rgb2yuv($input);
    if($html) {
      $output = 'yuv('.$output[0].','.$output[1].','.$output[2].')';
    }
    break;
  case 'yuv2rgb':
    $output = yuv2rgb($input);
    if($html) {
      $output = 'rgb('.$output[0].','.$output[1].','.$output[2].')';
    }
    break;
  ...
  case 'hsv2rgb':
    ...
    break;
  case 'hsl2hex':
    $output = rgb2hex(hsl2rgb($input));
    if($html) {
      $output = '#'.$output;
    }
    break;
  case 'hex2hsl':
    $output = rgb2hsl(hex2rgb($input));
    if($html) {
      $output = 'hsl('.$output[0].','.$output[1].'%,'.$output[2].'%)';
    }
    break;
  case 'complexity2int':
    $output = complexity2int($input);
    if($html) {
      $output = 'hsl('.$output[0].','.$output[1].'%,'.$output[2].'%)';
    }
    break;
  case 'int2rgb':
    $output = int2rgb($input);
    if($html) {
      $output = 'hsl('.$output[0].','.$output[1].'%,'.$output[2].'%)';
    }
    break;
  case 'complexity2rgb':
    $output = colorconv(colorconv($input, 'complexity2int'), 'int2rgb');
    break;
  case 'mixrgb':
    ...
    break;
  case 'parse':
    $output = parseColor($input);
    if($html) {
      $output = 'rgb('.$output[0].','.$output[1].','.$output[2].')';
    }
    break;
  default:
    $output = colorconv($input, 'parse');
    if($html) {
      $output = 'rgb('.$output[0].','.$output[1].','.$output[2].')';
    }
    break;
